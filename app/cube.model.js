var colorIdsList = {}; //List of all colors used to identify cubes; prevents the use of one colorID for two different cubes
...
class Cube {
  ...
  //Generates unique color that will identify the cube and will use for off-screen rendering
  colorIdGenerator(){
    let color = [Math.random(), Math.random(),Math.random(), 1.0];
    let key = color[0] + ':' + color[1] + ':' + color[2];

    if (key in colorIdsList){
      return colorIdGenerator();
    } else {
      colorIdsList[key] = true;
      return color;
    }
  }
  ...
  generateColor() {
    ...
    let   red   = _randomize("color"),
          green = _randomize("color"),
          blue  = _randomize("color");
    const alpha = 1.0;
